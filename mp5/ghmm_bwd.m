...
%   A		state transition probabilities [ from x to ]
...
for t=T-1:-1:1,
  ...
  % ndisp('beta from transition probs A, Gaussian probs Pys, and beta(t+1)')
  beta(:,t) = sum(transpose(bsxfun(@times,transpose(A),Pys(:,t+1).*beta(:,t+1))),2)/scale(t);
end
